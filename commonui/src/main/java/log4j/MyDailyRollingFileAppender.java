...
//   GMT (the epoch).
    ...
    int computeCheckPeriod() {
        ...
        if (datePattern != null) {
            for (int i = TOP_OF_MINUTE; i <= TOP_OF_MONTH; i++) {
                //将所示的时间格式化为当前时区
                @SuppressLint("SimpleDateFormat") SimpleDateFormat simpleDateFormat = new SimpleDateFormat(datePattern);
    ...
    void rollOver() throws IOException {
        ...
        if (file.getParentFile().exists()) {
            LogFileFilter logFileFilter = new LogFileFilter(file.getName());
            File[] files = file.getParentFile().listFiles(logFileFilter);
